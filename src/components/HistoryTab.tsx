...
import { Swords, Trophy, ArrowUp, Flame, ScrollText, Filter } from 'lucide-react';
import { Button } from '@/components/ui/button';

type FilterType = 'ALL' | 'CHALLENGE' | 'FRIENDLY' | 'INITIATION' | 'PROMOTION';
...
function getLogIcon(type: LogType) {
  switch (type) {
    ...
    case 'INITIATION': return <Trophy className="h-4 w-4 text-green-400" />;
    case 'PROMOTION': return <ArrowUp className="h-4 w-4 text-yellow-400" />;
  }
}

function getLogAccent(type: LogType): string {
  switch (type) {
    case 'CHALLENGE': return 'border-l-primary/60';
    case 'FRIENDLY': return 'border-l-orange-400/60';
    case 'INITIATION': return 'border-l-green-400/60';
    case 'PROMOTION': return 'border-l-yellow-400/60';
  }
}
...
function LogCard({ log }: { log: GlobalLog }) {
  ...
  return (
    <div
      className={`
        relative border-l-[3px] ${getLogAccent(log.type)} pl-4 py-3 pr-3
